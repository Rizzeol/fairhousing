...
var ChainStore = function () {
               ...
               if (optional_object) {
                  ...
                  if (delta < 60) {
                     _cjs.Apis.instance().db_api().exec("set_subscribe_callback", [_this.onUpdate.bind(_this), true]).then(function (v) {
                        ...
                        _cjs.Apis.instance().db_api().exec("set_pending_transaction_callback", [_this.onPendingTransaction.bind(_this), true]).then(function (res) {
                           resolve();
                        }).catch(function (err) {
                           reject(err);
   ...
   ChainStore.prototype.onPendingTransaction = function onPendingTransaction(data) {
      this.notifyPendingTransactionSubscribers(data);
   };

   ChainStore.prototype.notifyPendingTransactionSubscribers = function notifyPendingTransactionSubscribers(data) {
      var _this2 = this;
      ...
      if (!this.dispatched_pts) {
         ...
         this.timeout = setTimeout(function () {
            _this2.dispatched_pts = false;
            _this2.pendingTransactionSubscribers.forEach(function (callback) {
               callback(data);
            });
   ...
   ChainStore.prototype.notifySubscribers = function notifySubscribers(updated_objects) {
      var _this3 = this;
   ...
   ChainStore.prototype.subscribePendingTransaction = function subscribePendingTransaction(callback) {
         ...
         _cjs.Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
            ...
            for (var _i = 0; _i < optional_objects.length; _i++) {
               ...
               if (optional_object) _this7._updateObject(optional_object, true);else {
                  _this7.objects_by_id = _this7.objects_by_id.set(id, null);
            ...
            // in the event of an error clear the pending state for id
            console.log('!!! Chain API error', error);
            _this7.objects_by_id = _this7.objects_by_id.delete(id);
         });
      ...
      if (_ChainValidation2.default.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
         ...
         if (!_account2) {
            this.lookupAccountByName(id_or_account).then(function (account) {
               ...
               var account_id = account.get('id');
               var witness_id = _this8.witness_by_account_id.get(account_id);
               if (_ChainValidation2.default.is_object_id(witness_id)) return _this8.getObject(witness_id, on_update);

               if (witness_id == undefined) _this8.fetchWitnessByAccount(account_id).then(function (witness) {
                  _this8.witness_by_account_id.set(account_id, witness ? witness.get('id') : null);
                  if (witness && on_update) on_update();
               });
            }, function (error) {
               var witness_id = _this8.witness_by_account_id.set(id_or_account, null);
            });
         } else {
            ...
            if (_witness_id == undefined) this.fetchWitnessByAccount(_account_id).then(function (witness) {
               _this8.witness_by_account_id.set(_account_id, witness ? witness.get('id') : null);
               if (witness && on_update) on_update();
            });
         }
         return null;
   ...
   ChainStore.prototype.getCommitteeMember = function getCommitteeMember(id_or_account) {
      ...
      if (_ChainValidation2.default.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
         ...
         if (!account) {
            this.lookupAccountByName(id_or_account).then(function (account) {
               var account_id = account.get('id');
               var committee_id = _this9.committee_by_account_id.get(account_id);
               if (_ChainValidation2.default.is_object_id(committee_id)) return _this9.getObject(committee_id, on_update);

               if (committee_id == undefined) {
                  _this9.fetchCommitteeMemberByAccount(account_id).then(function (committee) {
                     _this9.committee_by_account_id.set(account_id, committee ? committee.get('id') : null);
                     if (on_update && committee) on_update();
                  });
               }
            }, function (error) {
               var witness_id = _this9.committee_by_account_id.set(id_or_account, null);
            });
         } else {
            var account_id = account.get('id');
   ...
   ChainStore.prototype.fetchFullAccount = function fetchFullAccount(name_or_id) {
      ...
      if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
         ...
         _cjs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
            if (results.length === 0) {
               if (_ChainValidation2.default.is_object_id(name_or_id)) {
                  _this12.objects_by_id = _this12.objects_by_id.set(name_or_id, null);
            ...
            _this12.accounts_by_name = _this12.accounts_by_name.set(account.name, account.id);
            ...
            */
            ...
            console.log("Error: ", error);
            if (_ChainValidation2.default.is_object_id(name_or_id)) _this12.objects_by_id = _this12.objects_by_id.delete(name_or_id);else _this12.accounts_by_name = _this12.accounts_by_name.delete(name_or_id);
         });
      }
      return undefined;
   };
   ...
   ChainStore.prototype.fetchRecentHistory = function fetchRecentHistory(account) {
      ...
      pending_request.promise = new Promise(function (resolve, reject) {
         _cjs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
            ...
            _this13.objects_by_id = _this13.objects_by_id.set(account_id, updated_account);
      ...
      approvals.forEach(function (id) {
         ...
         if (impactedAccount) {
            ...
            if (!proposals.includes(objectId)) {
               ...
               _this15._updateObject(impactedAccount.toJS());
            }
         }
      });
   };
